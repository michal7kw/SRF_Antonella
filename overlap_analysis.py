...
def parse_arguments():
    ...
    parser.add_argument('--promoter_upstream', type=int, default=2000,
                        help='Distance upstream of TSS to define promoter region')
    parser.add_argument('--promoter_downstream', type=int, default=200,
                        help='Distance downstream of TSS to define promoter region')
...
def extract_gene_coordinates(gtf_file, gene_ids, promoter_upstream=2000, promoter_downstream=200):
    """Extract promoter coordinates from GTF file for the given gene IDs."""
    print(f"Extracting promoter coordinates for {len(gene_ids)} genes from {gtf_file}")
    print(f"Defining promoters as {promoter_upstream}bp upstream to {promoter_downstream}bp downstream of TSS")
...
def find_overlaps(promoter_coords_df, peaks_by_sample):
    """Find overlaps between gene promoters and peaks."""
    print("Finding overlaps between gene promoters and peaks")
...
def plot_overlap_summary(summary, output_dir):
    ...
    plt.savefig(os.path.join(output_dir, 'promoter_overlap_summary.png'), dpi=300)
    plt.close()
    ...
    plt.savefig(os.path.join(output_dir, 'promoter_sample_similarity.png'), dpi=300)
    plt.close()
...
def main():
    ...
    promoter_coords_df = extract_gene_coordinates(
        args.gtf_file, all_de_genes, args.promoter_upstream, args.promoter_downstream
    )
    ...
    # Find overlaps
    overlaps_by_sample = find_overlaps(promoter_coords_df, peaks_by_sample)
