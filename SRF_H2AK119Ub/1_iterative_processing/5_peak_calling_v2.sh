...
#   with parameters suitable for broader marks like H2AK119Ub.
...
#
# Usage:
#   bash 5_peak_calling_v2.sh <sample_index>
# Example:
#   bash 5_peak_calling_v2.sh 0  # Processes the first sample (GFP_1)
#
# Parallel Execution:
#   Use GNU Parallel or xargs for parallel processing across samples:
#   N=6 # Number of parallel jobs
#   seq 0 5 | parallel -j $N bash 5_peak_calling_v2.sh {}
#   # OR
#   # printf "%s\n" {0..5} | xargs -I {} -P $N bash 5_peak_calling_v2.sh {}
#
# Requirements:
...
VIZ_MAX_FOLD=20             # Cap fold enrichment for scaling visualization score (lower for broader marks)
...
main() {
    ...
    # Step 2: MACS2 Peak Calling (Lenient Broad)
    log "Step 2: Running MACS2 peak calling (lenient) for ${sample_name}..."
    ...
    # Step 4: Filter Peaks (Lenient Criteria) and Standardize BED6+3 Format
    ...
    awk -v sample="$sample_name" \
        ...
        # Apply filtering criteria (Lenient)
...
# Single sample:
# bash 5_peak_calling_v2.sh 0

# Parallel using GNU Parallel:
# N=6; seq 0 5 | parallel -j $N bash 5_peak_calling_v2.sh {}
