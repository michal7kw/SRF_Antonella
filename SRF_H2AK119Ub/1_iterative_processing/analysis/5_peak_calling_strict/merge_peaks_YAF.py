...
def main():
    ...
    # Pattern for YAF broadPeak files
    pattern = os.path.join(directory, "YAF_*_broad_peaks_final.broadPeak")
    
    # Find all matching files
    input_files = sorted(glob.glob(pattern))
    
    if not input_files:
        print("Error: No YAF broadPeak files found")
        sys.exit(1)
    
    print(f"Found {len(input_files)} files: {', '.join(input_files)}")
    
    # Output file
    output_file = os.path.join(directory, "YAF.broadPeak")
