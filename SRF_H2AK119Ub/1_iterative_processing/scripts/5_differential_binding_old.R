...
perform_diffbind <- function() {
    ...
    samples <- data.frame(
        ...
        Replicate = c(2:3, 1, 3),
        bamReads = file.path("analysis/aligned_old", 
    ...
    # Create DiffBind object with parameters optimized for broad peaks
    ...
    # Count reads with parameters optimized for broad peaks
    ...
    dba_data <- dba.count(dba_data, 
                         ...
                         summits=FALSE)       # Disable summit calculation
    ...
    dba_data <- dba.normalize(dba_data, 
                             ...
                             background=TRUE)  # Add background normalization for broad peaks
    ...
    # Extract results with parameters suitable for broad peaks
    log_message("Extracting results...")
    dba_results <- dba.report(dba_data,
                             th=1,     
                             bCalled=FALSE,  
                             bNormalized=TRUE,
                             bSummarized=TRUE,  # Add summarized results for broad regions
                             bControlSubtracted=TRUE)  # Subtract background
