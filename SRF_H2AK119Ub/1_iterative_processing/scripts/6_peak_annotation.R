...
dba_to_granges <- function(dba_obj) {
    # Extract peaks data with better error handling
    tryCatch({
        peaks_df <- dba.report(dba_obj, bCounts=FALSE, bCalledDetail=TRUE)
        
        if (nrow(peaks_df) == 0) {
            stop("No peaks found in DBA object")
        }
        
        # Print debugging information
        print("DBA report summary:")
        ...
        if (!all(c("Chr", "Start", "End") %in% colnames(peaks_df))) {
            stop("Required columns (Chr, Start, End) not found in peaks data")
        }
        
        # Convert chromosome names to character if they're factors
        ...
        if ("Fold" %in% colnames(peaks_df)) {
            mcols(gr)$score <- peaks_df$Fold
        }
        if ("FDR" %in% colnames(peaks_df)) {
            mcols(gr)$FDR <- peaks_df$FDR
        }
        if ("Conc" %in% colnames(peaks_df)) {
            mcols(gr)$Conc <- peaks_df$Conc
        }
