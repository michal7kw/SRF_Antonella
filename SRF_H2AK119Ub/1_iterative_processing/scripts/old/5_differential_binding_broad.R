...
dir.create("analysis/diffbind_broad", recursive = TRUE, showWarnings = FALSE)
dir.create("analysis/annotation_broad", recursive = TRUE, showWarnings = FALSE)
...
samples <- data.frame(
    ...
    Peaks = file.path("analysis/peaks",
                     c(paste0("YAF_", 1:3, "_broad_peaks.broadPeak"),
                       paste0("GFP_", 1:3, "_broad_peaks.broadPeak"))),
...
saveRDS(dba_data, "analysis/diffbind_broad/all_peaks.rds")
saveRDS(dba_results, "analysis/diffbind_broad/significant_peaks.rds")

# Convert to data frame and save
df_results <- as.data.frame(dba_results)
write.csv(df_results, "analysis/diffbind_broad/differential_peaks.csv", row.names=FALSE)
...
saveRDS(peakAnno, "analysis/annotation_broad/peak_annotation.rds")
write.csv(as.data.frame(peakAnno), 
         "analysis/annotation_broad/peak_annotation.csv",
         row.names=FALSE)

# Generate annotation plots
pdf("analysis/annotation_broad/annotation_plots.pdf")
...
if (nrow(ego) > 0) {
    saveRDS(ego, "analysis/annotation_broad/go_enrichment.rds")
    write.csv(as.data.frame(ego), 
             "analysis/annotation_broad/go_enrichment.csv",
             row.names=FALSE)
    
    # Generate enrichment plot
    pdf("analysis/annotation_broad/go_enrichment_plot.pdf")
    dotplot(ego, showCategory=20)
    dev.off()
}

print("Differential binding analysis for broad peaks completed")
