...
# Get command line arguments - peak type (broad/narrow) and output directory
args <- commandArgs(trailingOnly = TRUE)
peak_type <- if(length(args) > 0) args[1] else "broad"
output_dir <- if(length(args) > 1) args[2] else "."

# Define the base directory for output files
base_dir <- file.path(output_dir)

# Create output directory structure
subdirs <- c("plots")
sapply(file.path(base_dir, subdirs), dir.create, recursive = TRUE, showWarnings = FALSE)

# Construct the paths to the input files
peaks_file <- file.path("analysis", "7_differential_binding", paste0("diffbind_", peak_type), "significant_peaks.rds")
annotation_file <- file.path("analysis", "8_annotation_and_enrichment", paste0("annotation_", peak_type), "peak_annotation.rds")

# Read input data
diff_peaks <- readRDS(peaks_file)
peak_annotation <- readRDS(annotation_file)
...
summary_data <- data.frame(
    ...
    value = c(length(diff_peaks),
             ...
             3,
             ifelse(exists("genomic_distribution"), sum(genomic_distribution@annoStat$Feature == "Promoter"), NA),
             ifelse(exists("genomic_distribution"), sum(genomic_distribution@annoStat$Feature == "Enhancer"), NA))
)
