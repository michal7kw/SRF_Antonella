...
# Documentation:
# This script performs differential binding analysis for CUT&Tag data using DiffBind (strict settings)
...
# 3. Runs differential binding analysis using a specialized R script (strict version)
# 4. Generates output files with differential binding results
# This version is adapted for local execution.

# Input files:
# - analysis/5_peak_calling_strict/{sample}_broad_peaks_final.broadPeak: Final peak calls for each sample (strict version)
...
# Output files:
# - analysis/7_differential_binding_strict/: Directory containing strict DiffBind results
# - logs/7_differential_binding_strict.out: Standard output log for local run
# - logs/7_differential_binding_strict.err: Error log for local run
...
# Define directories
OUTPUT_DIR="analysis/7_differential_binding_strict"
PEAKS_DIR="analysis/5_peak_calling_strict_v2"  # Note the _strict suffix
...
# Using samples with comparable peak counts for more reliable comparison
...
# Run R script for differential binding analysis (strict version)
log_message "Running strict differential binding analysis..."
...
if ! Rscript scripts/7_differential_binding_strict.R \
    ...
    "${SAMPLE_IDS}" \
    "${SAMPLE_CONDITIONS}" \
    "${SAMPLE_REPLICATES}" > "${LOG_DIR}/7_differential_binding_strict.out" 2> "${LOG_DIR}/7_differential_binding_strict.err"; then
    log_message "ERROR: Strict differential binding analysis failed. Check logs/${LOG_DIR}/7_differential_binding_strict.err"
    exit 1
fi

log_message "Strict differential binding analysis completed successfully"
