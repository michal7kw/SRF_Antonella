...
# For each sample, it:
# 1. Calls broad peaks using MACS2 with parameters suitable for general broad marks (e.g., H2AK119Ub)
...
TMPDIR="tmp/${SAMPLE_INDEX}_${TIMESTAMP}"
mkdir -p "$TMPDIR"
# chmod 777 "$TMPDIR" # Permissions might not be needed locally
export TMPDIR
...
unset PYTHONSTARTUP
# unset PYTHONPATH # Keep if needed locally
unset PYTHONHOME
...
OUTPUT_DIR="analysis/5_peak_calling"
...
echo "Processing ${sample} (Index: ${SAMPLE_INDEX}) for broad peak calling..."
...
# Step 1: Initial MACS2 peak calling with parameters optimized for CUT&Tag (Lenient for H2AK119Ub)
...
# Run MACS2 with the specified parameters (Removed env -i)
macs2 callpeak \
...
# Apply filtering criteria (Lenient: fold>=2, len 500-100000)
awk -v sample="$sample" 'BEGIN{OFS="\t"} {
    ...
    if(orig_fold >= 2 && len >= 500 && len <= 100000) {
        # Ensure name follows strict BED format
        name = sprintf("%s_peak_%d", sample, NR);
        name = gensub(/[^a-zA-Z0-9_]/, "_", "g", name);
...
awk 'BEGIN{OFS="\t"} {
    # Normalize score based on fold enrichment (cap at e.g., 20 for broader marks)
    max_fold = 20;
    ...
    # Output standard BED6 + 3 additional columns
...
# Use the temporary chr-prefix BAM for FRiP calculation
...
if [ "$peak_count" -gt 0 ]; then
    ...
    echo -e "Length\tFoldEnrichment" > $stats_txt
fi
