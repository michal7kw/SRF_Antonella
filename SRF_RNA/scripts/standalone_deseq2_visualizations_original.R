  ...
  if (length(top_degs_current_comp) > 0) {
    ...
    if (length(top_degs_current_comp_present) > 1) { # Need at least 2 genes to plot
      ...
      if (file.exists(dds_file_path)) {
        ...
        # Define color scale and breaks for heatmap
        lim <- max(abs(top_degs_counts_scaled), na.rm = TRUE) # Find max absolute value for symmetric scale
        ...
        my_breaks <- seq(-lim, lim, length.out = 101) # 101 breaks for 100 colors
