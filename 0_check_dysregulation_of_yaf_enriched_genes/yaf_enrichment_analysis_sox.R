...
gtf_file <- "/beegfs/scratch/ric.broccoli/kubacki.michal/COMMON_DATA/gencode.v43.basic.annotation.gtf"
...
yaf_sox_gene_list_file <- "/beegfs/scratch/ric.broccoli/kubacki.michal/SRF_H2AK119Ub_cross_V5/1_find_gene_lists_intersections/output/YAF_SOX.csv"
...
# Load the YAF/SOX gene list
cat("Loading YAF/SOX gene list...\n")
yaf_sox_genes <- read.csv(yaf_sox_gene_list_file)
# Ensure gene_id column exists and get IDs without version numbers
if ("gene_id" %in% colnames(yaf_sox_genes)) {
  yaf_sox_gene_ids <- sub("\\.[0-9]+$", "", yaf_sox_genes$gene_id)
  cat(sprintf("Loaded %d unique gene IDs from YAF/SOX list.\n", length(unique(yaf_sox_gene_ids))))
} else {
  stop("Error: 'gene_id' column not found in ", yaf_sox_gene_list_file)
}

# Filter DESeq results to include only genes from the YAF/SOX list
cat("Filtering DESeq results based on YAF/SOX list...\n")
initial_gene_count <- nrow(deseq_results)
deseq_results <- deseq_results %>%
  filter(gene_id_no_version %in% yaf_sox_gene_ids)
cat(sprintf("Retained %d genes out of %d after filtering.\n", nrow(deseq_results), initial_gene_count))

# 2. Filter for significant DEGs (padj < 0.05 and |log2FoldChange| > 1) among the filtered genes
...
cat('YAF Binding Analysis Summary (Filtered for YAF/SOX Intersection Genes)\n') # Updated title
cat('======================================================================\n\n')

cat(sprintf('Analysis restricted to genes present in: %s\n', yaf_sox_gene_list_file))
cat(sprintf('Total genes in list: %d\n', length(unique(yaf_sox_gene_ids))))
cat(sprintf('Genes from list found in DESeq results: %d\n\n', nrow(deseq_results)))
...
cat('\nConclusion:\n')
cat('(Note: Conclusions apply only to the subset of genes shared between YAF and SOX targets)\n') # Added note
if (!is.na(wilcox_test$p.value) && wilcox_test$p.value < 0.05) {
  if (mean(up_signal$log2FC_binding, na.rm = TRUE) > mean(down_signal$log2FC_binding, na.rm = TRUE)) {
    cat('Within the YAF/SOX gene set, YAF binding is significantly higher at promoters of upregulated genes compared to downregulated genes.\n') # Updated conclusion text
    cat('This suggests that for these shared target genes, YAF may function as a transcriptional activator.\n')
  } else {
    cat('Within the YAF/SOX gene set, YAF binding is significantly higher at promoters of downregulated genes compared to upregulated genes.\n') # Updated conclusion text
    cat('This supports the hypothesis that for these shared target genes, YAF binding to promoters may cause gene downregulation, suggesting a repressive function.\n')
  }
} else {
  cat('Within the YAF/SOX gene set, no significant difference in YAF binding was found between promoters of up and down-regulated genes.\n') # Updated conclusion text
  cat('This suggests that for these shared target genes, YAF binding alone may not be sufficient to determine gene expression direction,\n')
