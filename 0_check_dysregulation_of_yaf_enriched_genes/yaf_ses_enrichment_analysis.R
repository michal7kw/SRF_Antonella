...
cat("\nLoading SES peaks...\n")
ses_peak_files <- list.files(ses_peak_dir, pattern = "SES_rep.*_peaks\\.xls$", full.names = TRUE)
if (length(ses_peak_files) == 0) stop("No SES xls peak files found in ", ses_peak_dir)
# *** Important: Check and potentially adjust the 'format = "xls"' part in load_and_merge_peaks 
# based on the actual structure of the SES .xls files after running the script once. ***
ses_peaks_merged <- load_and_merge_peaks(ses_peak_files, format = "xls") 
