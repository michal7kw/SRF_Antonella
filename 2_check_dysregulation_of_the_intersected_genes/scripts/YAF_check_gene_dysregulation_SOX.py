...
and YAF_SOX_strict.csv by comparing with differential expression results.
"""
...
configs = [
    {
        "name": "Non-Strict",
        "gene_list_path": f'{BASE_PATH}/0_find_gene_lists_intersections/output/YAF_SOX.csv',
        "output_dir": f'{BASE_PATH}/2_check_dysregulation_of_the_intersected_genes/YAF_dysregulation_analysis_SOX'
    },
    {
        "name": "Strict",
        "gene_list_path": f'{BASE_PATH}/0_find_gene_lists_intersections/output/YAF_SOX_strict.csv',
        "output_dir": f'{BASE_PATH}/2_check_dysregulation_of_the_intersected_genes/YAF_dysregulation_analysis_SOX_strict' # Corrected output dir for strict
    }
]
...
def analyze_dysregulation(config_name, gene_list_file, output_dir, diff_expr_data):
    ...
    print(f"--- Starting Analysis for {config_name} (YAF SOX) ---")
    ...
    print(f"Dysregulation summary ({config_name} - YAF SOX):")
    print(dysreg_counts)
    ...
    plt.title(f'Dysregulation Categories for YAF_SOX ({config_name}) Genes')
    ...
    plt.title(f'Volcano Plot of YAF_SOX ({config_name}) Genes')
    ...
    if not top_dysreg.empty:
        ...
        plt.title(f'Top {len(top_dysreg)} Significantly Dysregulated Genes ({config_name} - YAF SOX)')
    ...
    print(f"Analysis complete for {config_name} (YAF SOX). Generated the following visualizations:")
...
print("--- All YAF SOX analyses finished ---")
