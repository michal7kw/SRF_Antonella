#!/usr/bin/env python3
"""
Script to check the type of dysregulation of genes listed in GFP_SOX.csv
and GFP_SOX_strict.csv by comparing with differential expression results.
"""
...
configs = [
    {
        "name": "Non-Strict",
        "gene_list_path": f'{BASE_PATH}/1_find_gene_lists_intersections/output/GFP_SOX.csv',
        "output_dir": f'{BASE_PATH}/2_check_dysregulation_of_the_intersected_genes/GFP_dysregulation_analysis_SOX'
    },
    {
        "name": "Strict",
        "gene_list_path": f'{BASE_PATH}/1_find_gene_lists_intersections/output/GFP_SOX_strict.csv',
        "output_dir": f'{BASE_PATH}/2_check_dysregulation_of_the_intersected_genes/GFP_dysregulation_analysis_SOX_strict'
    }
]
...
def analyze_dysregulation(config_name, gene_list_file, output_dir, diff_expr_data):
    ...
    print(f"--- Starting Analysis for {config_name} (SOX) ---")
    ...
    print(f"Dysregulation summary ({config_name} - SOX):")
    print(dysreg_counts)
    ...
    plt.title(f'Dysregulation Categories for GFP_SOX ({config_name}) Genes')
    ...
    plt.title(f'Volcano Plot of GFP_SOX ({config_name}) Genes')
    ...
    if not top_dysreg.empty:
        ...
        # Ensure gene_name is unique before setting as index, append count if not
        top_dysreg['gene_name_unique'] = top_dysreg['gene_name'] + top_dysreg.groupby('gene_name').cumcount().astype(str).replace('0', '')
        heatmap_data = top_dysreg[['gene_name_unique', 'log2FoldChange', 'padj']].set_index('gene_name_unique')
        
        sns.heatmap(heatmap_data[['log2FoldChange', 'padj']], cmap='coolwarm', annot=True, fmt='.2g') # Select only numeric columns for heatmap
        plt.title(f'Top {len(top_dysreg)} Significantly Dysregulated Genes ({config_name} - SOX)')
        plt.ylabel('Gene Name') # Add Y axis label for clarity
        plt.tight_layout()
    ...
    print(f"Analysis complete for {config_name} (SOX). Generated the following visualizations:")
...
print("--- All SOX analyses finished ---")
