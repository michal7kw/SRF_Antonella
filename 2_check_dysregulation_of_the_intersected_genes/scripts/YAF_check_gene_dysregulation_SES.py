...
and YAF_SES_strict.csv by comparing with differential expression results.
"""
...
def analyze_dysregulation(config_name, gene_list_file, output_dir, diff_expr_data):
    ...
    print(f"--- Starting Analysis for {config_name} (YAF SES) ---")
    ...
    print(f"Dysregulation summary ({config_name} - YAF SES):")
    print(dysreg_counts)
    ...
    plt.title(f'Dysregulation Categories for YAF_SES ({config_name}) Genes')
    ...
    plt.title(f'Volcano Plot of YAF_SES ({config_name}) Genes')
    ...
    if not top_dysreg.empty:
        ...
        plt.title(f'Top {len(top_dysreg)} Significantly Dysregulated Genes ({config_name} - YAF SES)')
    ...
    print(f"Analysis complete for {config_name} (YAF SES). Generated the following visualizations:")
...
print("--- All YAF SES analyses finished ---")
