...
process_data_and_save_results <- function(YAF_peaks, SES_peaks, promoters, output_file) {
...
main <- function() {
  ...
  SES_peak_file <- file.path(BASE_PATH, "SRF_SES_V5/results_data_from_ncbi_corrected/SES.broadPeak") # SES remains broadPeak
  ...
  output_file_strict <- file.path(output_dir_base, "YAF_SES_strict.csv")
  comparison_output <- file.path(output_dir_base, "YAF_SES_comparison.csv")
  ...
  # Parse SES peak file (done once for all analyses)
  SES_peaks <- read_broadpeak(SES_peak_file) # SES uses read_broadpeak
  
  # Process standard peaks
  cat("\n=== Processing Standard YAF vs SES Peak Calling ===\n")
  YAF_peaks_standard <- read_bed(YAF_peak_file_standard) # YAF uses read_bed
  standard_results <- process_data_and_save_results(YAF_peaks_standard, SES_peaks, promoters, output_file_standard)
  
  # Process strict peaks
  cat("\n=== Processing Strict YAF vs SES Peak Calling ===\n")
  YAF_peaks_strict <- read_bed(YAF_peak_file_strict) # YAF uses read_bed
  strict_results <- process_data_and_save_results(YAF_peaks_strict, SES_peaks, promoters, output_file_strict)
  
  # Find common genes between standard and strict YAF vs SES analyses
  common_genes_comparison <- intersect(standard_results$gene_id, strict_results$gene_id)
  cat("\nFound", length(common_genes_comparison), "genes common to both standard and strict YAF vs SES analyses\n")
