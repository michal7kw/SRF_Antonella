...
def main():
    ...
    # Pattern for SOX2 broadPeak files
    pattern = os.path.join(directory, "*SOX2_*_broadPeak.bed")
    
    # Find all matching files
    input_files = sorted(glob.glob(pattern))
    
    if not input_files:
        print("Error: No SOX2 broadPeak files found")
        sys.exit(1)
    
    print(f"Found {len(input_files)} files: {', '.join(input_files)}")
    
    # Output file
    output_file = os.path.join(directory, "SOX2.broadPeak")
